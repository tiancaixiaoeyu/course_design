...
io.on("connection", (socket) => {  //1. 监听：用户连接
  const userName = socket.handshake.query.userName || '访客';
  const userRole = socket.handshake.query.userRole || 'student';
  ...
  try {
    ...
    socket.emit("welcome", {   // 客户端发来connection请求，然后服务器发送welcome
    ...
    socket.on("joinRoom", (roomId, opts) => {
      ...
      socket.emit("roomJoined", {  // 告知所选房间的具体信息(摆设，房间内的用户)给对应的socket.id
        map: {
    ...
    // 3.  离开房间 
    socket.on("leaveRoom", () => {
      if (!room) {  // 确保在没有有效房间的情况下不会出错
        return;
      }
      socket.leave(room.id); // vs socket.join(room.id)
      room.characters.splice( // 找到离开的角色的id，删除[1]删除的数量
    ...
    socket.on("itemsUpdate", async (items) => {
      if (!character.canUpdateRoom) { // 用户没有更新房间的资格[即密码错误，看2.3]
        return;
      }
      ...
      room.characters.forEach((character) => { // 重新生成每个角色的位置
        character.path = [];
        character.position = generateRandomPosition(room);
      });
      io.to(room.id).emit("mapUpdate", {  // 向客户端发送地图更新，主要就是room.items更新了
        map: {
          gridDivision: room.gridDivision,
          size: room.size,
          items: room.items,
