...
export const draggedItemRotationAtom = atom(0); export const userRoleAtom = atom("student");
...
export const UI = () => {
  ...
  const setName = () => {
    if (inputValue.trim() && userRole) {
      // 使用setUserName更新userName的值
      setUserName(inputValue.trim());
      // 由于setState是异步的,需要在下一个事件循环中获取最新值
      setTimeout(() => {
        console.log("新的userName值:", inputValue.trim());
        socket.emit("characterNameUpdate", inputValue.trim());
      }, 0);
     
      socket.emit("characterNameUpdate", userName);
